<script setup>
const year = new Date().getFullYear()

const navLinks = [
  { label: 'Experiencias', href: '#tours' },
  { label: 'Identidad', href: '#identity' },
  { label: 'Galería', href: '#gallery' },
  { label: 'Contacto', href: '#contacto' }
]

function scrollTo(href) {
  document.getElementById(href.slice(1))?.scrollIntoView({ behavior: 'smooth' })
}
</script>

<template>
  <footer class="relative bg-brand-dark overflow-hidden">
    <!-- Mountain divider top -->
    <div class="relative w-full h-[40px] md:h-[60px] overflow-hidden bg-brand-dark">
      <svg class="absolute bottom-0 left-0 w-full" viewBox="0 0 1440 60" preserveAspectRatio="none" fill="none">
        <path d="M0 0L48 8C96 16 192 32 288 36C384 40 480 32 576 26C672 20 768 16 864 20C960 24 1056 36 1152 38C1248 40 1344 32 1392 28L1440 24V60H0Z" class="fill-brand-dark" />
      </svg>
    </div>
    <div class="relative border-t border-brand-cream/10 topo-pattern">
    <img
      src="/assets/tours/Cumbrecitariosubtecascada/grupo-paisaje.altura.webp"
      alt=""
      loading="lazy"
      decoding="async"
      class="absolute inset-0 w-full h-full object-cover opacity-[0.08]"
    />
    <div class="absolute inset-0 bg-gradient-to-b from-brand-dark/70 via-brand-dark/90 to-brand-dark"></div>
    <div class="relative max-w-7xl mx-auto px-5 md:px-10 lg:px-20 pt-16 pb-8">

      <!-- Contenido principal: 3 columnas -->
      <div class="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 lg:gap-16 mb-12">

        <!-- Columna izquierda: Marca -->
        <div class="md:col-span-1">
          <p class="font-heading text-2xl tracking-[0.2em] text-brand-orange uppercase mb-4">
            Trekking Cumbrecita
          </p>
          <h3 class="font-heading text-3xl md:text-4xl text-brand-white uppercase leading-tight">
            Descubrí los senderos más hermosos del Valle de Calamuchita
          </h3>
        </div>

        <!-- Columna central: Navegación -->
        <div class="md:col-span-1">
          <h4 class="font-heading text-xl text-brand-white uppercase mb-6">
            Descubrí
          </h4>
          <ul class="space-y-4">
            <li v-for="link in navLinks" :key="link.href">
              <a
                :href="link.href"
                class="flex items-center gap-2.5 py-1.5 text-brand-cream/70 text-base font-sans hover:text-brand-orange active:text-brand-orange transition-colors duration-300"
                @click.prevent="scrollTo(link.href)"
              >
                {{ link.label }}
              </a>
            </li>
          </ul>
        </div>

        <!-- Columna derecha: Contacto -->
        <div class="md:col-span-1">
          <h4 class="font-heading text-xl text-brand-white uppercase mb-6">
            Contacto
          </h4>
          <ul class="space-y-4">
            <li>
              <a
                href="https://wa.me/5493546453047"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-2.5 py-1.5 text-brand-cream/70 text-base font-sans hover:text-[#25D366] active:text-[#25D366] transition-colors duration-300"
              >
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                WhatsApp
              </a>
            </li>
            <li>
              <a
                href="https://www.instagram.com/trekking_cumbrecita/"
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center gap-2.5 py-1.5 text-brand-cream/70 text-base font-sans hover:text-[#E1306C] active:text-[#E1306C] transition-colors duration-300"
              >
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                </svg>
                Instagram
              </a>
            </li>
            <li>
              <a
                href="mailto:Cascadaelchorrillo.2018@gmail.com"
                class="flex items-center gap-2.5 py-1.5 text-brand-cream/70 text-base font-sans hover:text-brand-orange active:text-brand-orange transition-colors duration-300"
              >
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                </svg>
                Email
              </a>
            </li>
          </ul>
        </div>
      </div>

      <!-- Barra inferior -->
      <div class="border-t border-brand-cream/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <p class="text-brand-cream/50 text-sm font-sans text-center md:text-left">
          &copy; {{ year }} Trekking Cumbrecita. Todos los derechos reservados.
        </p>
        <p class="text-brand-cream/50 text-sm font-sans text-center md:text-right">
          Hecho por <strong class="text-brand-cream/80">Alexis Re</strong> &middot;
          <a
            href="https://wa.me/5493513446349"
            target="_blank"
            rel="noopener noreferrer"
            class="inline-flex items-center gap-1.5 py-1 hover:text-[#25D366] active:text-[#25D366] transition-colors duration-300"
          >
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
            </svg>
            351 344-6349
          </a>
        </p>
      </div>
    </div>
    </div>
  </footer>
</template>
